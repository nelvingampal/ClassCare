...
7. IN PROGRESS / desktop presentation only: the existing Care Alerts surface now uses the scoped sky-blue hierarchy while retaining its alert/inquiry tabs and actions. Run the complete relevant synthetic regression suite and secondary-route smoke checks before proposing a merge. Physical camera and equipment rehearsal remain required before demonstration readiness.
